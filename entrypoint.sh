...
if [ "$(id -u)" = "0" ]; then
    ...
    if command -v gosu >/dev/null 2>&1; then
        ...
        exec su -p -s /bin/bash appuser -c 'exec "$@"' -- "$0" "$@"
    fi
fi
...
export LOGIN_NAVIGATION_RETRIES="${LOGIN_NAVIGATION_RETRIES:-}"
export LOGIN_NAVIGATION_RETRY_DELAY_MS="${LOGIN_NAVIGATION_RETRY_DELAY_MS:-}"
...
export SAVE_CAPTCHA_DATASET="${SAVE_CAPTCHA_DATASET:-}"
export CAPTCHA_DATASET_DIR="${CAPTCHA_DATASET_DIR:-}"
